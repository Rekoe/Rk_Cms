...
<body>
	...
	<@p.form id="inputForm" action="update.rk" method="post" tableClass="input">
		...
		<@p.text label="permission.name" id="name" name="name" value="${permission.name}" required="true" class="requireField" maxlength="40"/><@p.tr/>
		<@p.td colspan="1" label="permission.permissionCategory"  required="true">
			<@p.select id="permissionCategoryId" name="permissionCategoryId" value=permission.permissionCategoryId listKey="id" listValue="name" list=obj required="true"/>
		</@p.td><@p.tr/>
		<@p.textarea label="permission.description" value="${permission.description?html}" id="description" name="description" cols="60" rows="4" class="textbox" required="true" class="requireField" maxlength="40"/><@p.tr/>
		<@p.th />
